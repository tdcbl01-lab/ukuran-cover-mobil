...
if menu == "🔍 Cari Ukuran Cover":
  ...
  if not st.session_state["logged_in"]:
    ...
    if mode_kelola == "➕ Tambah Data Baru":
      ...
      st.text_input("ID", value=str(next_id), disabled=True, label_visibility="collapsed")

      st.markdown("Merek <span style='color:red;'>*</span>", unsafe_allow_html=True)
      input_merek = st.selectbox("Merek", list_merek_excel, index=0, label_visibility="collapsed", accept_new_options=True)

      st.markdown("Model <span style='color:red;'>*</span>", unsafe_allow_html=True)
      input_model = st.text_input("Model", placeholder="Contoh: Avanza", label_visibility="collapsed")

      st.markdown("Tahun <span style='color:red;'>*</span>", unsafe_allow_html=True)
      input_tahun = st.text_input("Tahun", placeholder="Contoh: 2018-2023", label_visibility="collapsed")
      ...
      with st.form("form_tambah_sisa"):
        ...
        baru["Status"] = st.selectbox("Status", list_status_fix, label_visibility="collapsed")
        ...
        if st.form_submit_button("Simpan Data Baru"):
          # BACA LANGSUNG FILE FISIK TANPA CACHE
          if os.path.exists(EXCEL_FILE):
          ...
          if any(not str(baru.get(k)).strip() for k in kolom_wajib):
            st.error("❌ Gagal! Kolom wajib bertanda bintang merah harus diisi!")
          else:
            ...
            # SIMPAN MUTLAK KE EXCEL FISIK
            df_final.to_excel(EXCEL_FILE, index=False)

            sync_to_github_background(EXCEL_FILE)
            st.cache_data.clear()

            st.session_state["notif_sukses"] = "✅ Data berhasil ditambahkan dan tersimpan permanen ke Excel!"
            st.rerun()
    else:
        ...
        with st.form("form_edit"):
          ...
          for col in df.columns:
            if col not in ["ID", "Pilihan_Edit", "Merek", "Status"] + kolom_foto_list:
              ...
              if col in kolom_wajib:
                st.markdown(f"{col} <span style='color:red;'>*</span>", unsafe_allow_html=True)
                update[col] = st.text_input(col, value=val_lama, label_visibility="collapsed")
              else:
          ...
          st.markdown("Status <span style='color:red;'>*</span>", unsafe_allow_html=True)
          update["Status"] = st.selectbox("Status", list_status_fix, index=default_idx_status, label_visibility="collapsed")
