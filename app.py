...
# 1. DEFINISIKAN EXCEL_FILE PALING UTAMA DI ATAS
...
# Fungsi memuat data fresh (langsung baca file fisik)
def load_data_fresh():
    ...
    if mode_kelola == "➕ Tambah Data Baru":
      ...
      with st.form("form_tambah_sisa"):
        ...
        if st.form_submit_button("Simpan Data Baru"):
          df_live = load_data_fresh()

          duplikat_cek = df_live[
              (df_live["Merek"].str.strip().str.lower() == str(input_merek).strip().lower())
              & (df_live["Model"].str.strip().str.lower() == str(input_model).strip().lower())
              & (
                  (
                      df_live["Tahun"].str.strip().str.lower()
                      == str(input_tahun).strip().lower()
                  )
                  if input_tahun.strip() != ""
                  else True
              )
              & (df_live["ID"].astype(str).str.strip() != "")
          ]
          ...
          if any(not str(baru.get(k)).strip() for k in kolom_wajib):
            ...
            df_final = pd.concat([df_live, df_baru_item], ignore_index=True)
